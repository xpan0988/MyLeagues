  ...
  return <div className="page champion-profile">
    <QueryState loading={query.isPending} error={query.error}>
      {data ? <>
        ...
        <div className="profile-columns"><section className="data-section"><div className="section-heading"><h2>Core Build Paths</h2><span>FIRST 3 COMPLETED CORE ITEMS · ORDERED</span></div><div className="analytics-list">{data.coreBuilds.length ? data.coreBuilds.slice(0, 5).map((build, index) => <div className="analytics-row" key={index}><div className="icon-stack core-build-items">{build.items.slice(0, 3).map((item) => <EntityIcon key={item.id} entity={item} />)}</div><b>{build.games} games</b><span>{build.usageRate.toFixed(1)}% usage</span><span>{build.winRate.toFixed(1)}% WR</span></div>) : <p className="muted">No completed core items in this scope.</p>}</div></section>
