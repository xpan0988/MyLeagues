  ...
  return <div className="page champion-profile">
    <QueryState loading={query.isPending} error={query.error}>
      {data ? <>
        ...
        <section className="data-section"><div className="section-heading"><h2>Performance</h2></div><div className="metric-grid performance-grid"><Metric label="Average K / D / A" value={`${data.performance.averageKills.toFixed(1)} / ${data.performance.averageDeaths.toFixed(1)} / ${data.performance.averageAssists.toFixed(1)}`} /><Metric label="CS / min" value={data.performance.averageCsPerMinute.toFixed(1)} /><Metric label="Avg duration" value={formatMatchDuration(Math.round(data.performance.averageMatchDurationSeconds))} /><Metric label="Highest kills" value={data.performance.highestKills} /><MultiKillMetric values={data.performance} /></div></section>
        <section className="data-section"><div className="section-heading"><h2>Laning @10</h2><span>Laning data {data.laningAtTen.coveredGames} / {data.laningAtTen.eligibleGames} tracked eligible games</span></div><div className="metric-grid performance-grid"><Metric label="CS @10" value={laningValue(data.laningAtTen.averageCsAtTen)} detail="LANE MINIONS" /><Metric label="CS / min @10" value={laningValue(data.laningAtTen.averageCsPerMinuteAtTen)} /><Metric label="Gold @10" value={laningValue(data.laningAtTen.averageGoldAtTen, 0)} /><Metric label="XP @10" value={laningValue(data.laningAtTen.averageXpAtTen, 0)} /><Metric label="Level @10" value={laningValue(data.laningAtTen.averageLevelAtTen)} /></div></section>
        <LanePerformance title="Laning Performance" summary={data.lanePerformance} />
        <div className="profile-columns"><section className="data-section"><div className="section-heading"><h2>Core Build Combinations</h2><span>UP TO 3 COMPLETED ITEMS · CANONICAL</span></div><div className="analytics-list">{data.coreBuilds.length ? data.coreBuilds.slice(0, 5).map((build, index) => <div className="analytics-row" key={index}><div className="icon-stack core-build-items">{build.items.slice(0, 3).map((item) => <EntityIcon key={item.id} entity={item} />)}</div><b>{build.games} games</b><span>{build.usageRate.toFixed(1)}% usage</span><span>{build.winRate.toFixed(1)}% WR</span></div>) : <p className="muted">No completed core items in this scope.</p>}</div></section>
...
function MultiKillMetric({ values }: { values: { doubleKills: number; tripleKills: number; quadraKills: number; pentaKills: number } }) {
  const tiers = [["Double", values.doubleKills], ["Triple", values.tripleKills], ["Quadra", values.quadraKills], ["Penta", values.pentaKills]] as const;
  return <div className="metric metric-multi-kills"><span className="metric-label">Multi-kills</span><div className="multi-kill-grid">{tiers.map(([label, count]) => <span className="multi-kill-stat" key={label}><small>{label}</small><b>{count}</b></span>)}</div></div>;
}

function laningValue(value: number | null, digits = 1) { return value === null ? "—" : value.toFixed(digits); }
