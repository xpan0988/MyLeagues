...
export function HomePage() {
  ...
  return <div className="page">
    ...
    <div className="status-strip">
      ...
      <span>{data?.syncState.status === "syncing" ? `Syncing ${data.syncState.completed}/${data.syncState.total ?? "—"}` : data?.syncState.status === "error" ? "Sync unavailable" : "Local data ready"}</span>
      <button className="text-button" type="button" onClick={() => sync.mutate()} disabled={sync.isPending || data?.syncState.status === "syncing"}><RefreshCw size={13} /> Sync now</button>
    </div>
