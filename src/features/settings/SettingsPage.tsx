...
function SettingsForm({ initial }: { initial: SettingsDto }) {
  ...
  const save = useMutation({ mutationFn: backend.updateSettings, onSuccess: () => void queryClient.invalidateQueries() });
  ...
  return <form className="settings-form" onSubmit={submit}>
    ...
    <section className="settings-section"><div><h2>Official Client</h2><p>Provide RiotClientServices.exe. The launcher performs process detection and starts only official executables.</p></div><label className="wide-field">Executable path<input placeholder="C:\\Riot Games\\Riot Client\\RiotClientServices.exe" value={form.riotClientPath ?? ""} onChange={(event) => setForm({ ...form, riotClientPath: event.target.value || null })} /></label></section>
