...
use crate::db::repositories::timeline::{
    LaningSnapshot, TEN_MINUTE_MS, TimelineRepository, validate_snapshot,
};
use crate::error::AppResult;
use crate::riot::client::RiotApiClient;
use crate::riot::types::{RegionalRoute, TimelineFrameResponse, TimelineResponse};
use crate::services::lane_analysis;
...
pub async fn start_background(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        // Lane fact revision 1 is independent of the legacy V1 @10 queue: a
        // V1 error or completion must never be interpreted as its completion.
        let lane_result = lane_analysis::run(&database, &riot, &puuid, &route).await;
        coordinator.finish().await;
        let lane_updated = lane_result.as_ref().copied().unwrap_or(0);
        match result {
            Ok(updated) if updated > 0 || lane_updated > 0 => {
                let _ = app.emit("timeline-facts-changed", ());
            }
            Ok(_) => {}
            Err(error) => {
                tracing::warn!(target: "timeline", error = %error, "timeline enrichment paused; persistent queue will resume")
            }
        }
        if let Err(error) = lane_result {
            tracing::warn!(target: "lane_analysis", error = %error, "LaneScore fact enrichment paused; persistent queue will resume");
        }
    });
}
...
async fn run(
    ...
    let mut updated = 0;
    loop {
        if summary_sync.is_running().await {
            return Ok(updated);
        }
        let match_id = {
        ...
        let Some(participant_id) = participant_id else {
            let connection = database.connection()?;
            TimelineRepository::mark_unsupported(
                &connection,
                puuid,
                &match_id,
                "timeline metadata did not map the tracked PUUID to a participant",
            )?;
            continue;
        };
        let Some(frame) = ten_minute_frame(&response.info.frames) else {
            ...
            TimelineRepository::mark_unsupported(
                ...
                puuid,
                &match_id,
                "timeline ended before a ten-minute frame",
            )?;
            continue;
        };
        let Some(participant) = frame.participant_frames.get(&participant_id.to_string()) else {
            ...
            TimelineRepository::mark_unsupported(
                ...
                puuid,
                &match_id,
                "ten-minute frame omitted the tracked participant",
            )?;
            continue;
        };
        let snapshot = LaningSnapshot {
            ...
            lane_minions: participant.minions_killed,
            neutral_minions: participant.jungle_minions_killed,
            total_gold: participant.total_gold,
            experience: participant.xp,
        ...
        let mut connection = database.connection()?;
        if TimelineRepository::insert_snapshot(&mut connection, &snapshot)? {
            updated += 1;
        }
    }
}
...
fn ten_minute_frame(frames: &[TimelineFrameResponse]) -> Option<&TimelineFrameResponse> {
    frames
        .iter()
        .find(|frame| frame.timestamp == TEN_MINUTE_MS)
        .or_else(|| {
            frames
                .iter()
                .filter(|frame| frame.timestamp > TEN_MINUTE_MS)
                .min_by_key(|frame| frame.timestamp)
        })
}
...
mod tests {
    ...
    use crate::riot::types::{
        TimelineFrameResponse, TimelineInfoResponse, TimelineMetadataResponse, TimelineResponse,
    };
    use std::collections::HashMap;

    fn frame(timestamp: i64) -> TimelineFrameResponse {
    ...
    fn maps_historical_local_puuid_from_timeline_metadata_only_as_fallback() {
        let response = TimelineResponse {
            metadata: TimelineMetadataResponse {
                participants: vec!["other".into(), "local".into()],
            },
