...
mod tests {
    ...
    #[test]
    fn reset_removes_reconstructable_archive_but_preserves_configuration() -> Result<(), Box<dyn std::error::Error>> {
        let database = Database::open_in_memory()?;
        let mut connection = database.connection()?;
        SettingsRepository::new(&connection).update(&SettingsUpdate { game_name: "Player".into(), tag_line: "OCE".into(), account_region: "americas".into(), platform_region: "oc1".into(), riot_client_path: Some("/Applications/Riot Client.app/Contents/MacOS/RiotClientServices".into()) })?;
        AccountRepository::new(&connection).upsert(&Account { puuid: "p".into(), game_name: "Player".into(), tag_line: "OCE".into(), summoner_id: None, account_region: "americas".into(), platform_region: "oc1".into() })?;
        let record = MatchRecord { match_id: "OC1_1".into(), game_creation: 1, game_end_timestamp: None, game_duration_seconds: 700, queue_id: 420, game_version: "16.1.1".into(), patch: "16.1".into(), season_key: Some("2026".into()) };
        let player = PlayerMatch { match_id: "OC1_1".into(), puuid: "p".into(), participant_id: Some(1), champion_id: 1, win: true, kills: 1, deaths: 1, assists: 1, double_kills: 0, triple_kills: 0, quadra_kills: 0, penta_kills: 0, total_minions_killed: 70, neutral_minions_killed: 2, gold_earned: 4000, summoner_spell_ids: [4, 12], keystone_id: None, primary_style_id: None, secondary_style_id: None, final_items: vec![], rune_selections: vec![] };
        ...
        TimelineRepository::insert_snapshot(&mut connection, &LaningSnapshot { match_id: "OC1_1".into(), puuid: "p".into(), frame_timestamp_ms: 600_000, lane_minions: 70, neutral_minions: 2, total_gold: 4000, experience: 5000, level: 6 })?;
        drop(connection);

        MaintenanceService::new(&database).reset_local_archive()?;
        let connection = database.connection()?;
        for table in ["matches", "player_matches", "match_laning_snapshots", "sync_match_queue", "timeline_sync_queue", "sync_state", "career_aggregates", "champion_aggregates", "rank_snapshots", "champion_mastery"] {
            let count: i64 = connection.query_row(&format!("SELECT COUNT(*) FROM {table}"), [], |row| row.get(0))?;
            assert_eq!(count, 0, "{table} should be reset");
        }
        assert_eq!(AccountRepository::new(&connection).get()?.unwrap().puuid, "p");
        let settings = SettingsRepository::new(&connection).get()?;
        assert_eq!((settings.game_name.as_str(), settings.tag_line.as_str(), settings.platform_region.as_str(), settings.riot_client_path.as_deref()), ("Player", "OCE", "oc1", Some("/Applications/Riot Client.app/Contents/MacOS/RiotClientServices")));
        Ok(())
    }
}
