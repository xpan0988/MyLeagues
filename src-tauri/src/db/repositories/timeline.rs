...
impl TimelineRepository {
    ...
    pub fn participant_id(connection: &Connection, match_id: &str, puuid: &str) -> AppResult<Option<i64>> {
        Ok(connection
            .query_row(
    ...
    pub fn insert_snapshot(connection: &mut Connection, snapshot: &LaningSnapshot) -> AppResult<bool> {
        let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let inserted = transaction.execute(
            ...
            params![
                snapshot.match_id, snapshot.puuid, snapshot.frame_timestamp_ms,
                snapshot.lane_minions, snapshot.neutral_minions, snapshot.total_gold,
                snapshot.experience, snapshot.level,
            ],
        )?;
        transaction.execute(
    ...
    pub fn mark_error(connection: &Connection, puuid: &str, match_id: &str, error: &str) -> AppResult<()> {
        connection.execute(
    ...
    pub fn mark_unsupported(connection: &Connection, puuid: &str, match_id: &str, reason: &str) -> AppResult<()> {
        connection.execute(
...
pub fn validate_snapshot(snapshot: &LaningSnapshot) -> AppResult<()> {
    if snapshot.frame_timestamp_ms < TEN_MINUTE_MS || snapshot.lane_minions < 0 || snapshot.neutral_minions < 0 {
        return Err(AppError::RiotData("invalid ten-minute timeline fact".to_owned()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn seed(database: &Database) -> Result<(), Box<dyn std::error::Error>> {
        ...
        AccountRepository::new(&connection).upsert(&Account { puuid: "p".into(), game_name: "n".into(), tag_line: "t".into(), summoner_id: None, account_region: "americas".into(), platform_region: "oc1".into() })?;
        MatchRepository::ingest(&mut connection, &MatchRecord { match_id: "OC1_1".into(), game_creation: 1, game_end_timestamp: None, game_duration_seconds: 700, queue_id: 420, game_version: "16.1.1".into(), patch: "16.1".into(), season_key: Some("2026".into()) }, &PlayerMatch { match_id: "OC1_1".into(), puuid: "p".into(), participant_id: Some(1), champion_id: 1, win: true, kills: 0, deaths: 0, assists: 0, double_kills: 0, triple_kills: 0, quadra_kills: 0, penta_kills: 0, total_minions_killed: 0, neutral_minions_killed: 0, gold_earned: 0, summoner_spell_ids: [4, 12], keystone_id: None, primary_style_id: None, secondary_style_id: None, final_items: vec![], rune_selections: vec![] })?;
        Ok(())
    }

    #[test]
    fn timeline_facts_are_idempotent_and_coverage_is_honest() -> Result<(), Box<dyn std::error::Error>> {
        let database = Database::open_in_memory()?;
        seed(&database)?;
        let mut connection = database.connection()?;
        TimelineRepository::enqueue_eligible(&connection, "p")?;
        let snapshot = LaningSnapshot { match_id: "OC1_1".into(), puuid: "p".into(), frame_timestamp_ms: 600_000, lane_minions: 70, neutral_minions: 2, total_gold: 4000, experience: 5000, level: 6 };
        assert!(TimelineRepository::insert_snapshot(&mut connection, &snapshot)?);
        assert!(!TimelineRepository::insert_snapshot(&mut connection, &snapshot)?);
    ...
    #[test]
    fn interrupted_timeline_work_returns_to_the_persistent_queue() -> Result<(), Box<dyn std::error::Error>> {
        let database = Database::open_in_memory()?;
        seed(&database)?;
        let mut connection = database.connection()?;
        TimelineRepository::enqueue_eligible(&connection, "p")?;
        assert_eq!(TimelineRepository::claim_next(&mut connection, "p")?.as_deref(), Some("OC1_1"));
